...
const STAFF_NAMES = ['Owner Toko', 'Kasir 1', 'Kasir 2', 'Rina Kasir'];

function randomTime() {
  const h = String(8 + Math.floor(rand() * 12)).padStart(2, '0'); // toko buka 08–20
...
async function addProduct(product) {
  if (APP_CONFIG.USE_SUPABASE) {
    ...
    if (!storeId) throw new Error('Toko tidak terdeteksi. Silakan login ulang.');
    const { error } = await supabaseClient.from('products').insert({
      store_id: storeId, name: product.name, category: product.category,
...
/**
 * Quick Restock — menjumlahkan stok lama dengan jumlah barang masuk baru.
 * Mengembalikan { oldStock, addedQty, newStock, product }.
 */
async function restockProduct(id, addedQty) {
  if (!addedQty || addedQty < 1) throw new Error('Jumlah stok masuk harus lebih dari 0.');

  if (APP_CONFIG.USE_SUPABASE) {
    // Ambil stok lama dulu untuk ditampilkan di notifikasi (RPC sendiri atomik di server)
    const products = await getProducts();
    const before = products.find((p) => p.id === id);
    const oldStock = before ? before.stock : 0;

    const { data, error } = await supabaseClient.rpc('restock_product', { p_product_id: id, p_added_qty: addedQty });
    if (error) { console.error(error); throw new Error(error.message); }

    const product = { id: data.id, name: data.name, category: data.category, buyPrice: data.buy_price, sellPrice: data.sell_price, stock: data.stock };
    return { oldStock, addedQty, newStock: data.stock, product };
  }
  ...
  return { oldStock, addedQty, newStock, product };
}
...
async function getTransactions() {
  if (APP_CONFIG.USE_SUPABASE) {
    ...
    return data.map((t) => ({
      ...
      paymentMethod: t.payment_method, status: t.status, cashierName: t.cashier_name || '-',
    }));
  }
 ...
 * Di mode Supabase, ini memanggil fungsi database `create_transaction`
 * yang mengunci baris produk (FOR UPDATE) sehingga aman dari race-condition
 * saat 2 kasir input transaksi produk yang sama secara bersamaan.
...
async function addTransaction(trx) {
  if (APP_CONFIG.USE_SUPABASE) {
    const { data, error } = await supabaseClient.rpc('create_transaction', {
      p_trx_code: trx.trxCode, p_trx_date: trx.date, p_trx_time: trx.time, p_customer: trx.customer,
      p_product_id: trx.productId, p_qty: trx.qty, p_payment_method: trx.paymentMethod,
      p_status: trx.status, p_cashier_name: trx.cashierName,
    });
    if (error) { console.error(error); throw new Error(error.message); }

    // Ambil stok terbaru untuk keperluan notifikasi "stok tipis" di UI
    const { data: prod } = await supabaseClient.from('products').select('stock, name').eq('id', trx.productId).single();
    return { newStock: prod?.stock ?? null, product: { name: prod?.name || data.product_name } };
  }
  ...
  // Kurangi stok otomatis (mode demo — localStorage)
